...
function VisualPostCard({ post, title, href }: { post?: SitePost; title: string; href: string }) {
  ...
  return (
    <Link href={post?.slug ? `${href}/${post.slug}` : href} className="group relative block aspect-[4/5] overflow-hidden rounded-md bg-[#f3f2e4] text-black shadow-[0_3px_18px_rgba(18,55,42,0.12)]">
      ...
      <div className="absolute inset-x-5 bottom-5">
        <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#FBFADA]/90 text-[#436850]">
    ...
    <EditableSiteShell>
      <main className="bg-[#FBFADA] text-[#12372A]">
        <section className="mx-auto max-w-[1760px] px-4 py-10 sm:px-8 lg:px-12">
          <div className="grid gap-10 border-b border-[#ADBC9F] pb-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.2em] text-[#436850]"><Camera className="h-4 w-4" /> {pagesContent.about.badge}</p>
              <h1 className="mt-5 max-w-4xl text-5xl font-extrabold leading-[0.98] tracking-[-0.06em] sm:text-6xl">About {SITE_CONFIG.name}</h1>
              <p className="mt-6 max-w-2xl text-lg font-medium leading-8 text-[#436850]">{pagesContent.about.description}</p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/image" className="rounded-full bg-[#12372A] px-6 py-3 text-sm font-extrabold text-[#FBFADA]">Explore images</Link>
          ...
          <section className="grid gap-10 py-12 lg:grid-cols-[0.7fr_1.3fr]">
            <div>
              ...
              <p className="mt-3 text-sm font-medium leading-7 text-[#436850]">A public visual directory should feel direct, useful, and image-rich. This layout keeps that rhythm across every page.</p>
            </div>
            <div className="grid gap-5 md:grid-cols-3">
              {values.map((value) => (
                <article key={value.title} className="rounded-md border border-[#ADBC9F] bg-[#FBFADA] p-6 shadow-[0_3px_16px_rgba(18,55,42,0.08)]">
                  <CheckCircle2 className="h-5 w-5 text-[#436850]" />
                  <h3 className="mt-5 text-xl font-extrabold tracking-[-0.04em]">{value.title}</h3>
                  <p className="mt-3 text-sm font-medium leading-7 text-[#436850]">{value.description}</p>
                </article>
              ))}
            </div>
          </section>

          <section className="grid gap-6 border-t border-[#ADBC9F] pt-10 lg:grid-cols-3">
            {pagesContent.about.paragraphs.slice(0, 3).map((paragraph, index) => (
              <p key={paragraph} className={`text-base font-medium leading-8 text-[#436850] ${index === 0 ? 'lg:col-span-2' : ''}`}>{paragraph}</p>
            ))}
          </section>
        </section>
      </main>
