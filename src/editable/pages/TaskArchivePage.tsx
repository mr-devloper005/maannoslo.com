...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const archiveVars = { '--archive-bg': '#ffffff', '--archive-text': '#111111', '--archive-surface': '#ffffff', '--archive-accent': '#0057d9' } as CSSProperties
      ...
      <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-10 pt-8 sm:px-8 lg:px-12">
          <div className="flex flex-col gap-6 border-b border-slate-200 pb-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">{voice?.description || SITE_CONFIG.description}</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link href={basePath} className="rounded-full bg-black px-5 py-2.5 text-sm font-extrabold text-white">For You</Link>
              <Link href="/search" className="rounded-full bg-[#eaf4ff] px-5 py-2.5 text-sm font-extrabold">Explore</Link>
            </div>
          </div>

          <form action={basePath} className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.18em] text-slate-500"><Filter className="h-4 w-4" /> Filter</div>
            <select name="category" defaultValue={category} className="h-11 min-w-[220px] rounded-full border border-slate-300 bg-white px-4 text-sm font-bold outline-none">
              <option value="all">All categories</option>
              {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
            </select>
            <button className="h-11 rounded-full bg-[#3478f6] px-6 text-sm font-extrabold text-white">Apply</button>
            <p className="text-xs font-bold text-slate-500 sm:ml-auto">Showing: {categoryLabel}</p>
          </form>
        </section>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-8 lg:px-12">
          {task === 'image' || task === 'profile' ? (
            <div className="mb-6 flex items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm font-medium text-slate-600">{task === 'image' ? 'Browse image-first posts in a clean discovery grid' : 'Explore creative profiles and visual portfolios'}</p>
              </div>
              <Link href={basePath} className="hidden text-base font-extrabold text-[#0057d9] sm:inline">View All</Link>
            </div>
          ) : null}
          {posts.length ? (
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-extrabold">Previous</Link> : null}
            <span className="rounded-full bg-black px-5 py-3 text-sm font-extrabold text-white">Page {page} of {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-extrabold">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group block overflow-hidden rounded-md bg-white transition hover:-translate-y-1">
      <div className={`relative overflow-hidden bg-slate-100 ${index % 5 === 0 ? 'aspect-[16/12]' : index % 5 === 1 ? 'aspect-[16/10]' : 'aspect-[4/3]'}`}>
        <img src={image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
        {index % 4 === 0 ? <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/70 to-transparent p-4 pt-12 text-white"><span className="line-clamp-1 text-sm font-extrabold">{post.title}</span></div> : null}
      </div>
      <div className="flex items-center justify-between gap-3 px-1 py-3">
        <h2 className="line-clamp-1 text-sm font-extrabold">{post.title}</h2>
        <span className="inline-flex shrink-0 items-center gap-1 rounded bg-black/80 px-2 py-1 text-[10px] font-extrabold text-white"><ImageIcon className="h-3 w-3" /> {index + 1}</span>
      </div>
    </Link>
  )
}
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group rounded-md border border-slate-200 bg-white p-4 text-center shadow-[0_3px_14px_rgba(15,23,42,0.10)] transition hover:-translate-y-1 hover:shadow-xl">
      <div className="grid grid-cols-2 gap-3">
        {[0, 1, 2, 3].map((offset) => images[offset] ? <img key={offset} src={images[offset]} alt="" className="aspect-[4/3] rounded-md object-cover" /> : <div key={offset} className="flex aspect-[4/3] items-center justify-center rounded-md bg-slate-100"><UserRound className="h-6 w-6 text-slate-400" /></div>)}
      </div>
      <div className="mx-auto -mt-8 flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-slate-100 ring-4 ring-white">
      ...
      {role ? <p className="mt-2 line-clamp-1 text-sm text-slate-700">{role}</p> : null}
