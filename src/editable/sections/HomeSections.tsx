...
function SectionHeader({ title, subtitle, href }: { title: string; subtitle: string; href: string }) {
  return (
    <div className="mb-5 flex items-end justify-between gap-4">
      <div>
        <h2 className="text-xl font-extrabold tracking-[-0.04em] text-[#12372A]">{title}</h2>
        <p className="mt-2 text-sm font-medium text-[#436850]">{subtitle}</p>
      </div>
      <Link href={href} className="hidden items-center gap-2 text-base font-extrabold text-[#436850] sm:inline-flex">
...
function PhotoTile({ post, href, className = '', overlay = false, tall = false }: { post: SitePost; href: string; className?: string; overlay?: boolean; tall?: boolean }) {
  return (
    <Link href={href} className={`group relative block overflow-hidden rounded-md bg-[#f3f2e4] ${tall ? 'aspect-[4/3] sm:aspect-[3/4]' : 'aspect-[16/11]'} ${className}`}>
...
function QuestCard({ post, href, daysLeft }: { post: SitePost; href: string; daysLeft: number }) {
  return (
    <Link href={href} className="group block w-[310px] shrink-0 overflow-hidden rounded-md border border-[#ADBC9F] bg-[#FBFADA] p-4 shadow-[0_3px_14px_rgba(18,55,42,0.10)] sm:w-[420px]">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h3 className="line-clamp-1 text-base font-extrabold">{post.title}</h3>
        <span className="inline-flex items-center gap-1 text-sm font-medium text-[#436850]"><CalendarDays className="h-4 w-4" /> {daysLeft} days left</span>
      </div>
      <div className="relative aspect-[16/11] overflow-hidden rounded-md bg-[#f3f2e4]">
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      </div>
    </Link>
  )
...
function GalleryCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="group block w-[430px] shrink-0 rounded-md border border-[#ADBC9F] bg-[#FBFADA] p-4 shadow-[0_3px_16px_rgba(18,55,42,0.14)]">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-semibold">{getEditableCategory(post)}</h3>
        <span className="inline-flex items-center gap-1 rounded bg-[#436850] px-2 py-1 text-xs font-extrabold text-white"><ImageIcon className="h-3.5 w-3.5" /> {22 + index * 7}</span>
      </div>
  ...
  return (
    <Link href={href} className="group relative block w-[390px] shrink-0 overflow-hidden rounded-md bg-black text-white sm:w-[430px]">
      ...
      <div className="absolute inset-x-6 top-8">
        ...
        <div className="mt-5 h-1 w-12 rounded bg-[#FBFADA]" />
      ...
      {index > 0 ? <span className="absolute bottom-8 left-8 rounded bg-[#FBFADA] px-3 py-1 text-xs font-extrabold text-[#436850]">{35 + index * 19} NEW!</span> : null}
    </Link>
  )
}
    ...
    <section className={`${sectionShell} pb-14 pt-7`}>
      <div className="relative min-h-[380px] overflow-hidden rounded-2xl bg-black text-white lg:min-h-[430px]">
        ...
        <div className="relative z-10 flex min-h-[380px] max-w-3xl flex-col justify-center px-8 py-10 sm:px-16 lg:min-h-[430px]">
          ...
          <Link href="/signup" className="mt-7 inline-flex w-fit rounded-full bg-[#FBFADA] px-7 py-3 text-base font-extrabold text-[#12372A]">Sign up for free</Link>
        </div>
        ...
        <div className="grid h-[324px] grid-cols-3 gap-3 overflow-hidden bg-black p-10">
          {posts.slice(1, 7).map((post, index) => (
            <div key={post.id || post.slug} className={`relative overflow-hidden rounded ${index % 2 ? 'row-span-2' : ''}`}>
              ...
              <span className="absolute bottom-2 left-2 rounded bg-[#ADBC9F] px-2 py-1 text-[10px] font-extrabold text-[#12372A]">$100</span>
            </div>
          ))}
        </div>
        <div>
          ...
          <p className="mt-5 max-w-4xl text-lg font-medium leading-8 text-[#12372A]">Join contests, take on creative challenges, and showcase your talent. Improve your skills, gain exposure, and connect with people who care about strong visual work.</p>
          <Link href={primaryRoute} className="mt-7 inline-flex rounded-full bg-[#12372A] px-7 py-3 text-base font-extrabold text-[#FBFADA]">Sign up for free</Link>
          <div className="mt-9 flex gap-6 pl-4"><span className="h-1.5 w-4 rounded bg-[#12372A]" /><span className="h-1.5 w-1.5 rounded-full border border-[#ADBC9F]" /><span className="h-1.5 w-1.5 rounded-full border border-[#ADBC9F]" /></div>
        </div>
      </div>
    </section>
  )
  ...
  return (
    <>
      <section className={`${sectionShell} py-10`}>
        ...
        <div className="mx-auto h-1.5 w-44 rounded-full bg-[#f3f2e4]"><div className="h-full w-16 rounded-full bg-[#12372A]" /></div>
      </section>
      ...
      <section className={`${sectionShell} py-10`}>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {winners.slice(3, 11).map((post) => (
            <Link key={post.id || post.slug} href={postHref(primaryTask, post, primaryRoute)} className="group relative block aspect-[16/11] overflow-hidden rounded-md bg-[#f3f2e4]">
      ...
      <section className={`${sectionShell} py-10`}>
        ...
        <div className="flex gap-6 overflow-x-auto pb-8 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {active.slice(0, 5).map((post, index) => (
            <Link key={post.id || post.slug} href={postHref(primaryTask, post, primaryRoute)} className={`w-[330px] shrink-0 rounded-md border bg-[#FBFADA] p-4 text-center shadow-[0_3px_14px_rgba(18,55,42,0.12)] ${index === 1 ? 'border-[#436850] ring-1 ring-[#436850]' : 'border-[#ADBC9F]'}`}>
              <div className="grid grid-cols-2 gap-3">
                {[0, 1, 2, 3].map((offset) => <img key={offset} src={imageAt([post], offset)} alt="" className="aspect-[4/3] rounded-md object-cover" />)}
              </div>
              <img src={getEditablePostImage(post)} alt="" className="mx-auto -mt-8 h-16 w-16 rounded-full border-4 border-[#FBFADA] object-cover" />
              <h3 className="mt-4 text-xl font-extrabold">{post.title}</h3>
              <p className="mt-1 line-clamp-1 text-base text-[#436850]">{getEditableCategory(post)}</p>
              <span className="mt-6 inline-flex rounded-full bg-[#436850] px-7 py-3 text-base font-extrabold text-white">Follow</span>
            </Link>
          ))}
        </div>
        <div className="mx-auto h-1.5 w-52 rounded-full bg-[#f3f2e4]"><div className="h-full w-16 rounded-full bg-[#12372A]" /></div>
      </section>
    </>
  )
}
...
export function EditableHomeCta() {
  return (
    <section className={`${sectionShell} py-12`}>
      ...
      <form action="/search" className="mx-auto mt-4 flex max-w-xl rounded-full border border-[#ADBC9F] bg-[#FBFADA] p-1">
        ...
        <button className="inline-flex items-center gap-2 rounded-full bg-[#436850] px-5 py-3 text-sm font-extrabold text-white"><Search className="h-4 w-4" /> Search</button>
      </form>
    </section>
  )
}
